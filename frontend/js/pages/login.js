        ...
        <div class="w-full max-w-md relative">
            ...
            <div class="relative bg-v-charcoal border border-v-stone/20 rounded-2xl p-8">
                ...
                <div id="view-otp" class="hidden text-center">
                    ...
                    <button id="verify-btn" class="w-full py-2.5 bg-v-orange hover:bg-v-orange-h text-v-black font-semibold text-sm rounded-lg transition-colors mb-3">
                        Verify & Continue
                    </button>
                    <button id="back-to-auth" class="w-full py-2 text-sm text-v-stone-l hover:text-v-ash transition-colors">
                        ← Back to Sign In
                    </button>
                </div>
...
function initLogic() {
    ...
    formSignin.addEventListener('submit', async (e) => {
        ...
        try {
            const data = await api.post('/auth/login', { email: document.getElementById('si-email').value, password: document.getElementById('si-password').value });
            localStorage.setItem('token', data.token);
            localStorage.setItem('user', JSON.stringify(data.user));
            showToast('Login successful!', 'ok');
            window.location.reload();
        } catch (error) {
            const msg = Array.isArray(error.details) ? error.details[0] : (error.details?.[0]?.message || 'Login failed');
            // Special handling for 403 Unverified
            if (msg.includes('verify') || error.code === 'FORBIDDEN') {
    ...
    formSignup.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            showOtpView(email, data.otp); // Pass real OTP to display
            
        } catch (error) {
            const msg = Array.isArray(error.details) ? error.details[0] : (error.details?.[0]?.message || 'Signup failed');
            showToast(msg, 'err');
        } finally {
    ...
    document.getElementById('verify-btn').addEventListener('click', async () => {
        ...
        try {
            ...
            // Switch back to Sign In tab and pre-fill email
            ...
            const msg = Array.isArray(error.details) ? error.details[0] : (error.details?.[0]?.message || 'Verification failed');
            showToast(msg, 'err');
