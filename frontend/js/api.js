...
function handleMock(method, path, body) {
    const role = localStorage.getItem('role');
      ...
      // 1. Auth
    if (path === '/auth/signup') {
        // Backend now returns an OTP instead of a token
        const mockOtp = "1234"; 
        return { otp: mockOtp, message: "OTP sent to email" };
    }
    if (path === '/auth/verify') {
        // Simulate verification
        if (body.otp === "1234") {
            return { success: true, data: { verified: true }, message: "Email verified successfully" };
        } else {
            throw { details: [{ message: "Invalid OTP. Please try again." }] };
        }
    }
    if (path === '/auth/signin') {
        // Simulate unverified user blocking
        if (body.email === "unverified@test.com") {
            throw { code: "FORBIDDEN", details: ["Please verify your email"] };
        }
        return { token: 'mock_token', user: { id: 1, name: 'Mock User', email: body.email, role: 'hr', employee_id: 'EMP001' } };
    }
