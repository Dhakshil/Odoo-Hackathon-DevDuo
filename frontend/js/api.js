...
function handleMock(method, path, body) {
    ...
    const role = (localStorage.getItem('role') || user.role || 'employee').toLowerCase();

    if (path === '/auth/signup') {
        return { token: 'mock_token', user: { id: 1, name: body.name, email: body.email, role: body.role, employee_id: body.employee_id } };
    }

    if (path === '/auth/signin') {
        return { token: 'mock_token', user: { id: 1, name: 'Mock User', email: body.email, role: 'admin', employee_id: 'EMP001' } };
    }
    ...
    if (method === 'PATCH' && path.startsWith('/leaves/') && path.split('/').length === 3) {
        ...
        return { id, status: body.status, comment: body.comment };
    }
